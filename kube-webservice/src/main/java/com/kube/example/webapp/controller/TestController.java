...
import com.kube.example.webapp.entity.Articoli;
...
public class TestController {
	...
	public ResponseEntity<Articoli> listArtByCodArt(@PathVariable("codart") String codiceArticolo) {
		
		logger.info("Otteniamo l'articolo con codice: ["+codiceArticolo+"]");
	
		// lambda expression
		List<Articoli> articolo = this.initData()
			.stream()
			.filter(u -> u.getCodArt().equals(codiceArticolo))
	...
	public ResponseEntity<List<Articoli>> listArtByDesc(@PathVariable("filter") String filterString) {
		
		logger.info("Otteniamo l'articolo con descrizione: ["+filterString+"]");

		// lambda expression
		List<Articoli> articoli = this.initData()
			.stream()
			.filter(u -> u.getDescrizione().toUpperCase().contains(filterString.toUpperCase()))
			.collect(Collectors.toList());
	...
	public ResponseEntity<List<Articoli>> viewAll() {
		
		logger.info("Load all data on memory.");
		
		List<Articoli> articoli = this.initData();
		
		logger.info("Visualize all data on json format.");
